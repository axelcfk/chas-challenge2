...
export default function MyLists(){
  ...
  useEffect( () => {
    if (likedMoviesList && likedMoviesList.length > 0 || likedSeriesList && likedSeriesList.length > 0) {
  ...
  useEffect( () => {
    if (movieWatchList && movieWatchList.length > 0 || seriesWatchList && seriesWatchList.length > 0) {
  ...
  if (likedMoviesList == null || likedSeriesList == null) {
    return <div className="flex flex-col justify-center items-center md:items-start pb-10  px-8 md:px-20 h-screen w-screen bg-slate-950 text-slate-100">Loading like-lists...</div>
  }

  if (movieWatchList == null || seriesWatchList == null) {
    return <div className="flex flex-col justify-center items-center md:items-start pb-10  px-8 md:px-20 h-screen w-screen bg-slate-950 text-slate-100">Loading watch-lists...</div>
  }
  ...
  return (
    <div className="flex flex-col justify-center items-center md:items-start pb-10  px-8 md:px-20 h-screen w-screen bg-slate-950 text-slate-100">
      ...
      {
        likedMoviesList && likedMoviesList.length > 0 ? (<ul> {likedMoviesList.map((movie, index) => {
          return (
            <div className="pl-4" key={index}>ID: {movie.id}</div>
          )
        })} </ul>) : ("No movies liked yet")
       
      }

      
      <h2>Liked Detailed</h2>
      <button className="p-8 bg-slate-500" onClick={() => {
        setShowLikedDetails(true)
      }}>Show Details</button>

      { showLikedDetails ?
        (likedMoviesListDetails && likedMoviesListDetails.length > 0 ? (<ul> {likedMoviesListDetails.map((movie, index) => {
          return (
      ...
      <h2>Watchlist</h2>
      {
        movieWatchList && movieWatchList.length > 0 ? (<div> {movieWatchList.map((movie, index) => {
          return (
            <div className="pl-4" key={index}>ID: {movie.id}</div>
          )
        })} </div>) : ("No movies in watchlist")
      ...
      <button className="p-8 bg-slate-500" onClick={() => {
        ...
        {showWatchListDetails ? (movieWatchListDetails && movieWatchListDetails.length > 0 ? (<ul> {movieWatchListDetails.map((movie, index) => {
          return (
            <div className="pl-4" key={index}>
              <p>ID: {movie.id}</p>
              <p>title: {movie.title}</p>
