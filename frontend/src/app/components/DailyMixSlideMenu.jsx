...
export default function DailyMixBasedOnLikesSlideMenu() {
  ...
  const getGenerateDailyMixFromGPT = async () => {
    ...
    try {
      const response = await fetch(`${host}/api/generatedailymix`, {
        method: "GET",
  ...
  async function getMixFromOurDatabaseOnlyIDs() {
    try {
      const response = await fetch(`${host}/api/dailymixbasedonlikes`, {
  ...
  async function postAddToMixOnBackend(id, title) {
    try {
      ...
      const response = await fetch(`${host}/api/addtodailymixbasedonlikes`, {
        // users sidan på backend! dvs inte riktiga sidan!
        method: "POST",
        headers: {
          "Content-Type": "application/json",
